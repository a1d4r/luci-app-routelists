...
LuCI-приложение для OpenWrt: пользовательские списки (домены / IP / CIDR) для ZeroBlock как самостоятельная сущность — создание, редактирование, валидация и обзор в веб-интерфейсе, без SSH.

*A LuCI app for managing user list files (domains / IP / CIDR) for ZeroBlock — create, edit and validate lists right in the web UI.*

## Зачем

В ZeroBlock готовые «Списки сообщества» подключаются к любой секции, а свои домены и IP так не переиспользуешь: они вводятся в конкретную секцию или привязываются к ней файлом. Это приложение приближает свои списки к удобству списков сообщества: они создаются, редактируются и проверяются в LuCI (раздел **Службы → Пользовательские списки**), живут независимо от секций и подключаются к любой из них — путём к файлу.

## Скриншоты

**Таблица списков**

![Таблица списков](docs/screenshots/lists.png)

**Редактор с валидацией**
