...
ZeroBlock умеет подключать списки из файлов (поле «Пользовательские списки» в секции), но не даёт их вести: файлы создаются и правятся только по SSH. Это приложение добавляет в LuCI раздел **Службы → Пользовательские списки**, где все списки видны, редактируются и проверяются ещё до того, как файл попадёт в ZeroBlock.
